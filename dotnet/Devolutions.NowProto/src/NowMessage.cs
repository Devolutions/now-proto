    ...
    public class NowMessage
    {
        ...
        public readonly struct NowMessageView
        {
            ...
            public T Deserialize<T>() where T : INowDeserialize<T>
            {
                ...
                {
                    throw new NowDecodeException(NowDecodeException.ErrorKind.UnexpectedMessageClass);
                }

                if (_header.MsgKind != T.TypeMessageKind)
                {
                    throw new NowDecodeException(NowDecodeException.ErrorKind.UnexpectedMessageKind);
                }
        ...
        public NowMessageView Inspect()
        {
            return new NowMessageView(_header, _body);
        }
